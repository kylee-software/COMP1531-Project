...
def test_user_not_in_dm(clear, token, user_id_list):
    ...
    lastname = "lastname91"
    token2 = auth_register_v1(email,password,firstname, lastname)['token']
    dm = dm_create(token, user_id_list[1:])
    with pytest.raises(AccessError):
...
def test_everything_valid(clear, token, user_id_list):
    dm = dm_create(token, user_id_list[1:])
    dm_invite(token, dm['dm_id'], user_id_list[0])
    assert len(dm_details(token, dm['dm_id'])['members']) == 6
