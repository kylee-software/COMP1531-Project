...
def test_user_nonexistent():
    ...
    admin = auth_register_v2('test@unsw.au', 'password', 'first', 'last')
    channel_id = channels_create_v2(admin['auth_user_id'], 'channel_1', True)
    with pytest.raises(InputError):
...
def test_not_owner_of_channel_or_dreams():
    ...
    admin_1 = auth_register_v2('test@unsw.au', 'password', 'first', 'last')
    ...
    channel_id = channels_create_v2(admin_1['auth_user_id'], 'channel_1', True)
