from src.message import message_pin_v1, message_unpin_v1, message_send_v2, message_senddm_v1
import pytest
...
@pytest.fixture
def pin_message_channel(admin, channel_message):
    message_pin_v1(admin['token'], channel_message['message_id'])


@pytest.fixture
def pin_message_dm(admin, dm_message):
    message_pin_v1(admin['token'], dm_message['message_id'])


def test_invalid_message_id(admin, channel):
    with pytest.raises(InputError):
        message_unpin_v1(admin['token'], 123)


def test_message_already_unpinned_channel(admin, channel, channel_message):
    with pytest.raises(InputError):
        message_unpin_v1(admin['token'], channel_message['message_id'])


def test_message_already_unpinned_dm(admin, dm_message):
    with pytest.raises(InputError):
        message_unpin_v1(admin['token'], dm_message['message_id'])


def test_user_not_member_channel(channel_message, admin, not_member):
    with pytest.raises(AccessError):
        message_unpin_v1(not_member['token'], channel_message['message_id'])


def test_user_not_member_dm(dm_message, admin, not_member):
    with pytest.raises(AccessError):
        message_unpin_v1(not_member['token'], dm_message['message_id'])


def test_not_channel_owner(admin, member, channel_message):
    with pytest.raises(AccessError):
        message_unpin_v1(member['token'], channel_message['message_id'])


def test_not_dm_owner(admin, member, dm_message):
    with pytest.raises(AccessError):
        message_unpin_v1(member['token'], dm_message['message_id'])
...
def test_success_case_channel(admin, member, channel, channel_message):
    ...
    assert channel_message_dict['messages'][0]['is_pinned'] == False
...
def test_success_case_dm(admin, member, dm, dm_message):
    ...
    assert dm_message_dict['messages'][0]['is_pinned'] == False
