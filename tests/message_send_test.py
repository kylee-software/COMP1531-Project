...
def test_invalid_token(clear, token, channel_id):
    ...
    with pytest.raises(AccessError):
        message_send_v1(invalid_token, channel_id, 'testMessage')
...
def test_message_with_notification(clear, token, channel_id):
    msg_id = message_send_v1(token, channel_id, 'test message @firstNamelastName')
    assert isinstance(msg_id, int)

def test_invalid_channel_id(clear, token):
    with pytest.raises(InputError):
        message_send_v1(token, 'channel_id', 'test message')