...
def test_given_email_is_invalid():
    ...
    registration = requests.post(f"{config}/register/v2",
...
def test_email_already_exists():
    requests.delete(f"{config} /clear/v1")
    requests.post(f"{config}/register/v2",
                  json={'email': 'test@unsw.edu.au', 'password': 'password', 'name_first': 'test123', 'name_last': 'last123'})
    registration = requests.post(f"{config}/register/v2",
...
def test_registration_successful():
    requests.delete(f"{config} /clear/v1")
    registration = requests.post(f'{config}/register/v2', json={'email': 'test@unsw.au',
