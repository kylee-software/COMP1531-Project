...
def channel_message(admin, channel):
    return requests.post(config.url + '/message/send/v2', json={admin['token'], 'channel_id': ['channel_id'], 'message': 'this is a message sent to the other user in the channel.'}).json()
...
def test_invalid_token_dm(dm_message):
    ...
    message_call = requests.put(config.url + '/message/edit/v2', json={
        'token': invalid_token, 'message_id': dm_message['message_id'], 'message': 'this is an updated message in the dm.'})
...
def test_message_incorrect_length_dm(admin, dm_message):
    message_call = requests.put(config.url + '/message/edit/v2', json={
        'token': admin['token'], 'message_id': dm_message['message_id'], 'message': 1500*'A'})
...
def test_message_sent_by_unauthorised_user_and_not_channel_owner(member, channel_message):
...
def test_success_channel_message(admin, channel, channel_message):
    ...
    channel_messages = requests.get(config.url + '/channel/messages/v2', json={
        'token': admin['token'], 'message_id': channel['channel_id'], 'start': 0}).json
    assert channel_messages['messages'][0] == 'this edit is valid in this channel.'
...
def test_success_dm_message(admin, dm, dm_message):
    ...
    dm_messages = requests.get(config.url + '/dm/messages/v1', json={
                               'token': admin['token'], 'message_id': dm['dm_id'], 'start': 0})
    assert dm_messages['messages'][0] == 'this edit is valid in this dm.'
    assert edit.status_code == 200
